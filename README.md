# Data structures and algorithms for front-end developers

This repo provides a collection data structures and algorithms problems list along with the solution in JavaScript that one can refer or practice for front-end engineering interviews.

---

This repo is in progress

---

## Problems by category

### Array

1. [Running Sum of 1d Array](./array/1480.md)
1. [Missing Number](https://leetcode.com/problems/missing-number/)
1. [Single Number](https://leetcode.com/problems/single-number/)
1. [Kids With the Greatest Number of Candies](https://leetcode.com/problems/kids-with-the-greatest-number-of-candies)
1. [Shuffle the Array](https://leetcode.com/problems/shuffle-the-array)
1. [How Many Numbers Are Smaller Than the Current Number](https://leetcode.com/problems/how-many-numbers-are-smaller-than-the-current-number)
1. [Create Target Array in the Given Order](https://leetcode.com/problems/create-target-array-in-the-given-order)
1. [XOR Operation in an Array](https://leetcode.com/problems/xor-operation-in-an-array)
1. [Flipping an Image](https://leetcode.com/problems/flipping-an-image)
1. [Maximum Product of Two Elements in an Array](https://leetcode.com/problems/maximum-product-of-two-elements-in-an-array)
1. [Number of Students Doing Homework at a Given Time](https://leetcode.com/problems/number-of-students-doing-homework-at-a-given-time)
1. [Fibonacci Number](https://leetcode.com/problems/fibonacci-number)
1. [Minimum Absolute Difference](https://leetcode.com/problems/minimum-absolute-difference)
1. [Majority Element](https://leetcode.com/problems/majority-element)
1. [Move Zeroes](https://leetcode.com/problems/move-zeroes)
1. [Find All Numbers Disappeared in an Array](https://leetcode.com/problems/find-all-numbers-disappeared-in-an-array)
1. [Missing Number](https://leetcode.com/problems/missing-number)
1. [Best Time to Buy and Sell Stock](./array/121.md)
1. [Min Cost Climbing Stairs](https://leetcode.com/problems/min-cost-climbing-stairs)
1. [Remove Element](https://leetcode.com/problems/remove-element)
1. [Maximum Subarray](https://leetcode.com/problems/maximum-subarray)
1. [Remove Duplicates from Sorted Array](https://leetcode.com/problems/remove-duplicates-from-sorted-array)
1. [Two Sum](https://leetcode.com/problems/two-sum)
1. [Check If It Is a Straight Line](https://leetcode.com/problems/check-if-it-is-a-straight-line)
1. [Third Maximum Number](https://leetcode.com/problems/third-maximum-number)
1. [Combination Sum](https://leetcode.com/problems/combination-sum)
1. [Unique Paths](https://leetcode.com/problems/unique-paths)
1. [Container With Most Water](https://leetcode.com/problems/container-with-most-water)
1. [Minimum Number of Days to Make m Bouquets](https://leetcode.com/problems/minimum-number-of-days-to-make-m-bouquets)
1. [Find Peak Element](https://leetcode.com/problems/find-peak-element)
1. [Search a 2D Matrix](https://leetcode.com/problems/search-a-2d-matrix)
1. [Word Search](https://leetcode.com/problems/word-search)
1. [Spiral Matrix](https://leetcode.com/problems/spiral-matrix)
1. [Search in Rotated Sorted Array](https://leetcode.com/problems/search-in-rotated-sorted-array)
1. [Unique Paths II](https://leetcode.com/problems/unique-paths-ii)
1. [3Sum](https://leetcode.com/problems/3sum)
1. [Non-decreasing Array](https://leetcode.com/problems/non-decreasing-array)

### Linked List

1. [Add Two Numbers](./linked-list/2.md)
1. [Delete Node in a Linked List](./linked-list/237.md)
1. [Middle of the Linked List](./linked-list/876.md)
1. [Merge Two Sorted Lists](./linked-list/21.md)
1. [Remove Duplicates from Sorted List](./linked-list/83.md)
1. [Reverse Linked List](./linked-list/206.md)
1. [Intersection of Two Linked Lists](./linked-list/160.md)
1. [Linked List Cycle](./linked-list/141.md)
1. [Palindrome Linked List](./linked-list/234.md)
1. [Remove Linked List Elements](./linked-list/203.md)
1. [Swapping Nodes in a Linked List](./linked-list/1721.md)
1. [Odd Even Linked List](./linked-list/328.md)
1. [Reorder List](./linked-list/143.md)
1. [Remove Nth Node From End of List](./linked-list/19.md)
1. [Rotate List](./linked-list/61.md)
